...
def create_project():
    ...
    if cov and cov.startswith('data:'):
        ...
        # External URL (e.g. auto-fetched thumbnail) — download & save locally
        try:
            r2 = proxy_one(cov)
            if r2 and r2.startswith('data:'):
                cover_url = save_dataurl(r2, ALLOWED_IMG, uid)
                if cover_url == '__STORAGE_LIMIT__':
                    cover_url = cov  # fallback to external URL
            else:
                cover_url = cov  # keep external URL as-is
        except:
            cover_url = cov  # keep external URL on error
    else:
        cover_url = None
...
def update_project(pid):
    ...
    def resolve_file(field, old_url, allowed):
        ...
        if v.startswith(('http://','https://')):
            # External URL — download & save locally
            try:
                r2 = proxy_one(v)
                if r2 and r2.startswith('data:'):
                    delete_file(old_url, uid)
                    url = save_dataurl(r2, allowed, uid)
                    return v if url == '__STORAGE_LIMIT__' else url
            except: pass
            return v  # keep external URL on failure
        return old_url
...
@login_required
def video_thumbnail():
    """Fetch video thumbnail from any supported platform (oEmbed where available)."""
    url = (request.get_json() or {}).get('url','').strip()
    if not url:
    ...
    try:
        ...
        if yt:
            vid = yt.group(1)
            # YouTube thumbnails are predictable - try hqdefault first, fallback to default
        ...
        if 'tiktok.com' in url:
            try:
                body, _ = fetch_url(f'https://www.tiktok.com/oembed?url={url}')
                oe = json.loads(body)
                return jsonify({'thumbnail': oe.get('thumbnail_url',''), 'title': oe.get('title','')})
            except: pass
        # Instagram - oEmbed requires app token, but we can try og:image scrape
        if 'instagram.com' in url:
            try:
                body, _ = fetch_url(url)
                og = re.search(r'<meta property="og:image"\s+content="([^"]+)"', body)
                if og:
        ...
        return jsonify({'error': str(ex), 'thumbnail':'', 'title':''}), 200  # Don't fail hard
